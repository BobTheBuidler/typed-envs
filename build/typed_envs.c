...
PyInit_typed_envs(void)
{
    ...
    tmp = PyImport_ImportModuleLevel("typed_envs__mypyc", NULL, NULL, fromlist, 0);
    if (!tmp) return NULL;
    ...
    if (deps_capsule != NULL) {
        int (*deps_func)(void) = (int (*)(void))PyCapsule_GetPointer(
            deps_capsule, "typed_envs__mypyc.ensure_deps");
    ...
    PyObject *capsule = PyObject_GetAttrString(tmp, "init_typed_envs");
    Py_DECREF(tmp);
    if (capsule == NULL) return NULL;
    void *init_func = PyCapsule_GetPointer(capsule, "typed_envs__mypyc.init_typed_envs");
    Py_DECREF(capsule);
    if (!init_func) {
        return NULL;
    }
...
PyMODINIT_FUNC PyInit___init__(void) { return PyInit_typed_envs(); }
